...
# The function used by django to get the midpoints of a group based on their location text.
...
def getPlaces(midpoint,radius=1000):
    api_key="AIzaSyD4ULRyhwhux8lmfESkwICF4ZJSJnlQesI"
    gmaps = googlemaps.Client(key=api_key)
    fields = ['formatted_address','geometry','name']
    types = ['restaurant','bar','night_club','meal_takeaway']
    results  = gmaps.places_nearby(location=midpoint,language="en-AU",min_price=1,max_price=5,open_now=True,type=type,radius=radius)
    radius = 800
    while(len(results)==0):
        print('no results')
        radius = radius + 200
